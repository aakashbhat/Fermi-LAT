...
In this work we consider the classification of gamma-ray sources into two classes -- active galactic nuclei (AGNs) and pulsars -- as well as into three classes 
...
This problem is connected to the selection of a hard detection threshold of $TS = 25$ for the 3FGL and 4FGL %\LEt{ Is there a difference between 4FGL and 4FGL-DR2? Either way, this different nomenclature should be explained.}
...
Thus, on the one hand, a lower threshold can be useful in studies where a more complete list of sources is desirable and a higher false detection rate is admissible.%\LEt{ Verify that your intended meaning has not been changed.} 
...
We also show how the probabilistic catalogs can be used for population studies of sources, for example,%\LEt{ We do not allow the use of "e.g." or "i.e." within the main text (in parentheses or within figure/table captions is fine).} 
...
The general workflow will have the following three steps: (1) Select data for training and testing. (2) Optimize algorithms using training data sets.
...
where the Spectral\_Index %\LEt{ Quotation marks can be used to introduce a special meaning for a word or phrase the first time it is used in the text, but thereafter quotation marks are not needed. Please check for this throughout}
...
Sign\_Avg (significance %\LEt{ significant?}
...
and the rest 346 sources are OTHER sources, such as pulsar wind nebulas PWN%\LEt{ Please define.}
