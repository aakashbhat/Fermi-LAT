...
We find that in the two-class \LEt{ Write out whole numbers when lower than 11 (i.e., zero to ten) and not directly used as a measurement with the unit following.}case it is important to correct for the presence of OTHER sources among the unassociated ones in order to realistically estimate the number of pulsars and AGNs.
...
\citep[4FGL-DR2;][]{2020ApJS..247...33A, 2020arXiv200511208B}. \LEt{ Verify that your intended meaning has not been changed.}We used the versions gll\_psc\_v16.fit for 3FGL
...
Bayesian association probabilities were also included in the 4FGL\LEt{ should this be "3FGL"?.} \citep{2020ApJS..247...33A} and
...
This problem is connected to the selection of a hard detection threshold of $TS = 25$ for the 3FGL and 4FGL \LEt{ Is there a difference between 4FGL and 4FGL-DR2? Either way, this different nomenclature should be explained.}catalogs.
...
Thus, on the one hand, a lower threshold can be useful in studies where a more complete list of sources is desirable and a higher false detection rate is admissible.\LEt{ Verify that your intended meaning has not been changed.} On the other hand, a lower threshold can be problematic for studies where 
a clean sample is necessary. 
The problem of detection threshold selection can be ameliorated with the development of a probabilistic catalog.
...
We also show how the probabilistic catalogs can be used for population studies of sources, for example,\LEt{ We do not allow the use of "e.g." or "i.e." within the main text (in parentheses or within figure/table captions is fine).} as a function of their flux or position on the sky, where one includes not only associated sources but also unassociated ones according to their class probabilities.
...
In Sect. \ref{sec:pop_studies} we show applications of the probabilistic catalogs for predicting the number of pulsars, AGNs, and OTHER sources among the unassociated sources and in the construction of the source counts as a function of their flux, $N(S)$, and as a function of 
...
in Appendix \ref{sec:app_O_vs_S} we compare the oversampling method used in the paper with Synthetic Minority Over-sampling TEchnique (SMOTE) \LEt{ Please remember to spell out acronyms upon first appearance in the abstract and then again beginning with the introduction. All subsequent appearances should be in the acronym form (unless at the beginning of sentence).}oversampling,
...
For the input data we take associated point sources in the 3FGL or 4FGL-DR2 catalogs, which we then split into training and testing subsets.
We consider four ML algorithms: random forests \citep[RF;][]{709601, Breiman:2001hzm}, 
...
Thus for training and testing, we sub-select the sources, which are associated with pulsars and AGNs.
...
\LEt{ A\&A does not allow bulleted or numbered lists outside of the conclusion, so I have reformatted accordingly.}The general workflow will have the following three steps: (1) Select data for training and testing. (2) Optimize algorithms using training data sets.
We select meta-parameters of the algorithms by optimizing the accuracy of the classification and test for overfitting using the test data sets.
In order to get stable results, we repeat the separation of the data into training and testing samples 100 times and 
average the accuracy. (3)
...
where the Spectral\_Index \LEt{ Quotation marks can be used to introduce a special meaning for a word or phrase the first time it is used in the text, but thereafter quotation marks are not needed. Please check for this throughout}feature is the parameter $\al$, that is, the tilt in the spectrum at the pivot energy $E_0$ (which also varies for different sources).
...
Sign\_Avg (signif \LEt{ significant?}average), Pivote\_E (pivot energy), FD (flux density), Un\_FD (uncertainty on flux density), F1000 (Flux\_1000), Un\_F1000 (Uncertainty\_Flux\_1000),
...
For the classification we use BDT with 100 trees and the maximum depth of 2.
...
The unknown parameters are optimized by minimizing a loss function, which we chose to be
the cross entropy
...
averaged over 100 random splits into training and testing samples.
We use two neurons in the hidden layer, the tanh activation function, and the LBFGS solver. 
}
...
For training with oversampling, we copy randomly existing pulsars and add them to the data set until the number of pulsars and AGNs are the same.
Although pulsars in the training data set are redundant, they help to increase the weight of pulsars in the classification model.
...
\includegraphics[width=0.5\textwidth]{plots/classification_domains/lr_200_lbfgs.pdf}
\caption{\LEt{ Please provide a title for this figure.}Top panel: LR classification domains showing class probabilities for training with oversampling.
...
In addition to original data sets, we perform oversampling of pulsars in order to balance the numbers of pulsars and AGNs.
...
We replace the missing and unphysical values according to the procedure described at the beginning of Sect. \ref{sec:training}.
...
We notice \LEt{ A\&A encourages authors to avoid directly addressing the reader, e.g., by using the imperative or posing direct questions.}that the last source is classified as a pulsar by BDT and RF algorithms and as an AGN by LR and NN algorithms.
...
For the later comparison with the three-class classification, we also add unassociated source in the 3FGL catalog, 
...
The precision (recall) estimates using, for example, the condition that all algorithms agree is computed by taking the ratio of associated pulsars in 3FGL, which are also predicted to be pulsars by all eight algorithms, to the number of sources associated with pulsars (to the total number of sources predicted to be pulsars among associated sources) in the 3FGL catalog.
...
\renewcommand{\arraystretch}{1.5}
    ...
    %\vspace{2mm}
    \tablefoot{
    The classes are predicted using two-class classification.
    The precision and recall are calculated using all-algorithms-agreement condition and 4FGL-DR2 associations (4FGL assoc), or the class probabilities of associated 3FGL sources derived from test samples using either all-algorithms-agreement condition (all agree) or the $\sum_a p_a > 7$ condition ($\sum_a p_a > 7$).
...
and the rest 346 sources are OTHER sources, such as PWN\LEt{ Please define.}, supernova remnants (SNRs), and so on.
...
We use the same 11 features as in the two-class classification: the only difference is that we use cos(GLON) instead of GLON.
...
We perform optimization of the meta-parameters for the four ML algorithms with the three classes.
...
For NN, the accuracy stops increasing above about ten neurons in the hidden layer (in the following we use 11 neurons for classification: the same as in the two-class case).
For oversampling, we use the oversampling factors $\sqrt{\frac{\text{\# AGN}}{\text{\# PSR}}}$ and $\sqrt{\frac{\text{\# AGN}}{\text{\# OTHER}}}$ for the PSR and OTHER classes, respectively (compared to the $\frac{\text{\# AGN}}{\text{\# PSR}}$ oversampling factor in the two-class case).
...
As in the two-class case, the accuracies are averaged over 1000 realizations of splitting the data into training and testing samples.
...
Similarly to the two-class classification, we use the condition that all algorithms agree to determine the candidate probabilistic classes of sources.
...
A comparison of predicted classes (using the all algorithms agreement condition) for the 3FGL unassociated sources in the three-class classification case with the classes of the corresponding associated sources in the 4FGL-DR2 catalog are presented in Table~\ref{tab:3FGL_vs_4FGL_3class}.
...
we use 16 features instead of 11 features in the 3FGL case (the features are the same as in the two-class classification of 4FGL-DR2 sources in Sect. \ref{sec:4FGLprediction} but with GLON replaced by cos(GLON))
...
with the sum of PSR-class probabilities in the three-class classification larger than 7.
The PSR candidates are shown in Table \ref{tab:psr_candidates_3class}. 
All of these sources are also among the list of 29 PSR candidates determined in the two-class classification using both 3FGL and 4FGL-DR2 
features in Sect. \ref{sec:4FGLprediction}. 
For convenience, we also add the sums of PSR-like probabilities for the three-class classification of the 4FGL-DR2 sources
...
Unassociated 4FGL-DR2 sources with the sum of PSR-class probabilities for all eight ML methods in the three-class classification larger than 7. 
...
\textbf{4FGL J1847.7-0125}: Within 1 arcminute of the candidate Young Stellar Object (YSO) SSTOERC G031.2256+00.1711 \citep{2017ApJ...839..108S}.

\textbf{4FGL J1843.7-0326}: Associated with 3FGL J1843.7-0322 and found near the HESS source HESS J1843-033, next to the SNR G28.6-0.1 \citep{2018A&A...612A...1H}. This source is
...
\textbf{4FGL J1556.8-5242c}: Within 1 arcminute of the candidate YSO 2MASS J15564953-5241450 \citep{2008AJ....136.2413R}.

\textbf{4FGL J1631.7-4826c} (3FGL J1632.4-4820; \citealt{2015ApJS..218...23A}): Within 30 arcseconds of the Dark Cloud (Nebula) SDC G335.894-0.184 \citep{2016A&A...590A..72P}.
...
\textbf{4FGL J1849.4-0117} (3FGL J1849.5-0124c; \citealt{2015ApJS..218...23A}): It is in the region of Galactic mini starburst W43 studied by \citet{2020A&A...640A..60Y} and is also within 1 arcminute of the candidate YSO SSTOERC G031.5367-00.1555. It has a ``MIXED'' classification based on the 3FGL values.

\textbf{4FGL J1109.4-6115e} (3FGL J1111.9-6038; \citealt{2015ApJS..218...23A}): It is associated with the extended galactic source FGES J1109.4-6115 \citep{2017ApJ...843..139A}, near the speculated SFR 4FGL J1115.1-6118, in there region of Young Massive Stellar Cluster NGC 3603 \citep{2020ApJ...897..131S}. It has a ``MIXED'' classification with the 3FGL values.

\textbf{4FGL J1850.2-0201}: It is also in the region of the starburst W43 \citep{2020A&A...640A..60Y}.

\textbf{4FGL J1801.8-2358}: Associated with HESS J1800-240A and 2FHL J1801.7-2358. Located south of the SNR W28 \citep{2020MNRAS.495.2909R}.

\textbf{4}\textbf{FGL J1855.8+0150}: In the region of SNR W44 \citep{2020ApJ...896L..23P}

\textbf{4FGL J1742.0-3020}: Within 1 arcminute of the Molecular cloud [MML2017] 777 \citep{2017ApJ...834...57M}.

\textbf{4FGL J1904.7+0615}: Within 1 arcminute of the Bubble [SPK2012] MWP1G040012-001102 \citep{2012MNRAS.424.2442S}.
\end{enumerate}
...
In this subsection we determine the numbers of AGNs, pulsars, and, in case of three-class classification, OTHER sources as a function of their flux.
...
We see that predictions for the numbers of sources using latitude binning in the two-class case is closest to the three-class case.
...
The probabilities are calculated with eight different ML methods: RF, BDT, LR, and NN -- each algorithm with and without oversampling during training.
...
Most of the misclassified sources in this comparison have spectral parameters in 3FGL that are typical of the OTHER class (Fig. \ref{fig:3FGL_vs_4FGL_classes}); in other words, the misclassification can be due to problems with reconstructing the spectrum of the sources.
...
accuracy of the algorithms. For these tests we use the two-class classification in the 3FGL catalog.
...
Similarly to feature importances for the 3FGL two-class classification reported in Table \ref{tab:feat_imp}, 
...
We estimate the probabilities of classification of all sources in the 3FGL and 4FGL-DR2 catalogs using the same algorithms and meta-parameters as in Sect. \ref{sec:oversampling}.
...
In particular, we calculate the difference in probabilities of belonging to, for example, the PSR class relative to the maximal standard deviation
...
\caption{Precision and recall for different choices of the threshold for the classification of sources by individual algorithms.
The all-algorithms-agree method is used for the final classification. 
The precision and recall are calculated for unassociated 3FGL sources that have associations in the 4FGL-DR2 catalog.
...
Classification by the largest probability is a common approach in ML and we use it
