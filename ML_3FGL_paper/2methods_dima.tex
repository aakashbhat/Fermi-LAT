...
The accuracy increases with higher number of epochs and saturates at around 50 for L-BFGS and 300 for ADAM. As expected, L-BFGS converges faster than Adam, which uses stochastic gradient descent.
...
In general one can either under- or oversample in a dataset. Undersampling would reduce the number of AGNs to match the number of pulsars. However, since the total number of sources is not very high, we have chosen oversampling. 
