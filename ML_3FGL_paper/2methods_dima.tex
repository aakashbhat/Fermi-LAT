...
the maximal depth of trees for unweighted training data.
...
\dima{Comment here about the choice of the number of trees?}
...
It is important to note that this plot is for one run only (we split the data into training and testing samples only one time) and the model is trained on two features.
...
Unlike the RF, which is also an ensemble based method, the accuracy drops for the maximal depths larger than 2. 
