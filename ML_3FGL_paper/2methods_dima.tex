...
We notice that the accuracy does not decrease as the maximal depth of the trees increases, i.e., there is no overfitting as the complexity of the model increases with increased maximal depth.
% with a larger complexity of the model, e.g., there is little change in accuracy for maximal depths above $\sim$ 5.
...
We used the Sklearn Gradient Boosting algorithm for the construction of BDT (\cite{gb}).
%\dima{is there a name of the algorithm?}
...
The accuracy increases with higher number of epochs and saturates at around 50 for L-BFGS and 300 for ADAM. L-BFGS converges faster than Adam, which uses stochastic gradient descent.
...
for different solvers, e.g., LBFGS (discussed in the NN section, \cite{lbfgs}), SAG (Stochastic Average Gradient, \cite{sag}), SAGA (a variant of SAG, \cite{saga}),
and liblinear (a special solver for LR and support vector machine classifications, \cite{ll}).
%\dima{refs?}
...
all-algorithm classification for pulsar oversampling in training. For details see Section \ref{sec:oversampling}.}
...
Since the overall number of AGNs is larger than the number of pulsars, the testing accuracy with oversampling is smaller than the one without oversampling: 0.87 vs 0.95.
