...
This is in fact a single layer perceptron, or a neural network with one hidden layer, with several input nodes (each node corresponds to
...
For the final algorithm, we use a RF with 20 trees and a maximum depth of 12 trained on inversely weighted data.
...
What is the objective function: Gini index, entropy, or misclassification?}
...
In order to illustrate the separation of the PS into AGNs and pulsars, we retrain the RF algorithm using only two features: curvature significance and spectral index, and plot the resulting probabilities of classes in Figure \ref{fig:RF_domains}.
\dima{Add discussion of the domains, e.g., overfitting, and selection of the final model for classification.}

\begin{figure}[h]
%\centering
\includegraphics[width=\onepic\textwidth]{plots/classification_domains/rf_200_12.pdf}
\includegraphics[width=\onepic\textwidth]{plots/classification_domains/rf_20_2.pdf}
...
\dima{Since there are only ten features, one hidden layer and one output node, NNs with more than 10 nodes
in the hidden layer are equivalent to NN with 10 hidden layers: the dimension of the space of linear combinations of input features is at most 10. If there are more nodes in the hidden layer, then the model is degenerate. So in this case it doesn't make sense to plot more than 10 neurons in the hidden layer, we could also plot up to 20 and comment on the degeneracy of the model. I'm not sure I understand where you use Tanh or RELU activations, if we have only one hidden layer with sigmoid activation (I guess) for classification?}}
...
Classification domains are presented in Figure \ref{fig:LR_domains}.

\begin{figure}[h]
%\centerin
\includegraphics[width=\twopicsp\textwidth]{plots/classdom_lr_100_lbfgs.pdf}
\includegraphics[width=\twopicsp\textwidth]{plots/classdom_lr_200_saga.pdf}
\caption{Classification domains for Logistic Regression with lbfgs (top) and saga (bottom) solvers.
