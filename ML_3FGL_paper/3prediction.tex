...
%We have also subselected the 303 unassociated 3FGL sources, which have PSR or AGN associations in 4FGL-DR2, and saved them for convenience of comparison as a separate file.
...
\dima{Check the sources, in the earlier version there were different sources in the table.}
...
\dima{Should we add this information to the 2-class and 3-class PSR candidate tables and remove this table?}
...
columns/Separation/.style={column name=Sep (arcsec),numeric type,fixed,precision=1}
]\loadedtable
...
\dima{I'd move this table to Section \ref{sec:4FGLprediction}, where we discuss the 29 candidates, keep only the 4 sources which 
are still unassociated in 4FGL-DR2 and use the source names and coordinates in 4FGL-DR2.}}
\end{table}
...
\dima{I'd suggest to keep only the 4 PSR candidates, which are still there in the 4FGL-DR2 and add them as a column in the PSR candidates file.}
