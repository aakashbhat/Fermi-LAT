...
For the classification of 4FGL-DR2 sources, we checked whether the parameters used in 3FGL classification were still optimal and find most of them to be the same. Therefore we used the same meta-parameters for the four algorithms as in the construction of the probabilistic catalog based on 3FGL, except for NN where we increased the number of neurons in the hidden layer to 16. Similar to the construction of the 3FGL probabilistic catalog, we use both unweighted training samples and oversampling, i.e., we have 8 classification methods.
...
Finally, we looked at sources which were unassociated in both 3FGL and 4FGL-DR2 (using 'ASSOC\_FGL' as an identifier for 3FGL sources). Out of 303 such sources\footnotetext{These are actually 303 4FGL-DR2 sources and 302 3FGL sources with 2 4FGL-DR2 sources associated with 1 3FGL source.}, 40 sources are predicted to be pulsars using 3FGL features and 75 sources are predicted to be pulsars using 4FGL features. This leads to 29 sources which are predicted by all eight methods to be pulsars for features taken from both 3FGL and 4FGL catalogs. Among these 29 sources, 6 are spatially associated with pulsars in the Parkes survey based only on 3FGL parameters. Four of these are are also predicted to be pulsars when we add in the 4FGL-DR2 predictions. Of the other two, one is now associated as a pulsar in 4FGL-DR2, and the second one is not detected in 4FGL. % \dima{This is confusing, aren't we looking at unassociated sources in 4FGL, how can a source be then associated or undetected in 4FGL-DR2? Also I'm finding that there are 303 sources unassociated both in 3FGL and 4FGL-DR2.}
