...
We train on 70\% of the sources associated with pulsars or AGNs and save the probability values for training sources, for sources which are not classified as pulsars or AGNs, and for unassociated sources.
...
while the average for AGNs and pulsar is over the number of times the sources appear in the training sample, which is 300 on average.
...
For the classification of 4FGL sources, we do not perform another optimization of meta-parameters, i.e., we used the same meta-parameters for the four algorithms as in the construction of the probabilistic catalog based on 3FGL, except for NN where we increased the number of neurons in the first hidden layer to 16. Similarly to the construction of the 3FGL probabilistic catalog, we use both unweighted training samples and oversampling, i.e., we have 8 classification methods.
