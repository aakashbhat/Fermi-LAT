...
\renewcommand{\arraystretch}{1.5}
    ...
    or the test samples with 3FGL associations (``all agree'' labels) as well as $\sum_a p_a > 7$ condition for test samples
    with 3FGL associations (``$\sum_a p_a > 7$'' labels).
    The AGN and PSR sources are also represented in Fig. \ref{fig:3FGL_vs_4FGL_classes}.}
    \label{tab:3FGL_vs_4FGL_2class}
\end{table}
...
\renewcommand{\arraystretch}{1.5}
    ...
    \end{tabular}%}
    \vspace{2mm}
    \caption{The expected precision and recall for the classification of associated 4FGL-DR2 sources
