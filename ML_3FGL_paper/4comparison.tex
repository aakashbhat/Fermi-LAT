...
\caption{Cumulative number of sources as a function of their flux. Purple bands show the envelope of the sum of class probabilities for associated sources, while green bands show the sum of counts of associated sources (blue solid line) plus the sum of probabilities for unassociated sources. The curves with ``SP16'' in the labels are derived from the data in \cite{2016ApJ...820....8S}. For details see Section \ref{sec:dNdS}.}  
\label{fig:logN_logS}
\end{figure*}
...
The purple bands show the envelope of the sum of corrected probabilities for the eight methods used in this paper.
...
more consistent with the counts of pulsars.
%We note that the expected number of associated pulsars in 3FGL and 4FGL catalogs (purple band) is slightly below the actual counts of pulsars, this can be due to the fact that the number of AGNs in the training sample is much larger than the number of pulsars. \cite{2016ApJ...820....8S} have used weights in the training sample inversely proportional to the number of elements in the class, i.e., a pulsar has more weight in training than an AGN. This can be a reason for an overprediction of the number of associated pulsars, compared to the actual count.
...
The green bands show the corresponding envelopes for the eight ML methods.
