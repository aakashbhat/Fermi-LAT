...
\renewcommand{\arraystretch}{1.5}
    ...
    with associations in the 4FGL-DR2 catalog. 
    For the definition of precision and recall labels see Table \ref{tab:3FGL_vs_4FGL_2class}.
}
    \label{tab:3FGL_vs_4FGL_3class}
\end{table}
...
As one can from Table \ref{tab:selected_algs_multi} and also from the comparison of Tables \ref{tab:3FGL_vs_4FGL_3class}
and \ref{tab:3FGL_vs_4FGL_2class}, the sum of probabilities larger than 7 condition provides a better precision than the 
agreement among the algorithms condition.
In this section we use the $\sum_a p_a > 7$ condition to create lists of PSR and OTHER candidates
