...
cannot handle multinomial \dima{or multi-class?} loss.
}
...
Connection of unassociated 4FGL-DR2 sources predicted to be OTHER with Simbad database within 1 arc minute of the sources. We list only sources which have a seperate match than their 4FGL or 3FGL name.}
\end{table}
We have saved this candidate list with the rest of the catalogs. Furthermore, there are 6 PSR candidates in our multi-class classification with a probability sum $\text{PSR\_TOTAL}> 7.0$. These are shown in table \ref{tab:psrpredicted}. All of these sources are also predicted to be pulsars as part of our 29 PSR sources mentioned in section 4 (using both 4FGL and 3FGL features). 2 of these are also present in the Parkes survey mentioned before. Finally, the source 4FGL J1225.9+2951 is the only one with a Simbad match with a candidate white dwarf Gaia DR2 4011915928852958592 within 1 arc minute.

\pgfplotstableread[col sep=comma]{tables/4FGLDR2_PSR.csv}\loadedtable
\begin{table}
\tiny
\pgfplotstabletypeset[columns={Source_Name_4FGL,GLON,GLAT,PSR_TOTAL},
column type=l,
string type,
every head row/.style={before row={\hline \hline},after row=\hline,},
every last row/.style={after row=\hline},
...
columns/GLON/.style={column name=GLON,numeric type,fixed,precision=2},
columns/GLAT/.style={column name=GLAT,numeric type,fixed,precision=2},
columns/PSR_TOTAL/.style={column name=Sum(Prob),numeric type,fixed,precision=2},
]\loadedtable
\normalsize
\caption{\label{tab:psrpredicted}
Connection of unassociated 4FGL-DR2 sources predicted to be PSR by the multi class prediction. Sum(Prob) here represents the sum of PSR class probabilities}
\end{table}
