...
#test_out=input_1[1300:1600,5:]
    ...
    clf = RandomForestClassifier(n_estimators=num,oob_score=True,random_state=0) #n_estimators=no. of trees
    clf1 = ExtraTreesClassifier(n_estimators=num,oob_score=True,bootstrap=True,random_state=0)
  #  clf2 = GradientBoostingClassifier(n_estimators=num, learning_rate=1.0,max_depth=1, random_state=0).fit(train, train_truth)
    clf3 = AdaBoostClassifier(n_estimators=num,random_state=0)
...
fig,ax=plt.subplots()
plt.plot(num1,score,'go--', linewidth=2, markersize=10)
ax.set_xlabel('num_estimator')
ax.set_ylabel('score')
plt.show()
